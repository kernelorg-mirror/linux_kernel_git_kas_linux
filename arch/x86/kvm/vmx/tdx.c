...
static void tdx_flush_vp(void *_arg)
{
	...
	if (to_tdx(vcpu)->state != VCPU_TD_STATE_UNINITIALIZED) {
		...
		if (unlikely(!IS_TDX_ERR_VCPU_NOT_ASSOCIATED(err))) {
			/*
...
void tdx_mmu_release_hkid(struct kvm *kvm)
{
	...
	if (IS_TDX_ERR_FLUSHVP_NOT_DONE(err))
		goto out;
...
static fastpath_t tdx_exit_handlers_fastpath(struct kvm_vcpu *vcpu)
{
	 ...
	 */
	if (unlikely(IS_TDX_ERR_OPERAND_BUSY(vp_enter_ret)))
...
fastpath_t tdx_vcpu_run(struct kvm_vcpu *vcpu, bool force_immediate_exit)
{
	...
	if (unlikely(IS_TDX_ERR_SW_ERROR(tdx->vp_enter_ret)))
		return EXIT_FASTPATH_NONE;
			    ...
			    enum pg_level level, struct page *page)
{
	...
	if (unlikely(IS_TDX_ERR_OPERAND_BUSY(err))) {
		tdx_unpin(kvm, page);
		return -EBUSY;
	}
...
int tdx_handle_exit(struct kvm_vcpu *vcpu, fastpath_t fastpath)
{
	...
	/*
	 * Handle TDX SW errors, including TDX_ERR_SEAMCALL_UD,
	 * TDX_ERR_SEAMCALL_GP and TDX_ERR_SEAMCALL_VMFAILINVALID.
	 */
	if (unlikely(IS_TDX_ERR_SW_ERROR(vp_enter_ret))) {
	...
	if (unlikely(tdx_failed_vmentry(vcpu))) {
		...
		WARN_ON_ONCE(vcpu->arch.guest_state_protected &&
			     !IS_TDX_ERR_NON_RECOVERABLE(vp_enter_ret));
	...
	WARN_ON_ONCE(exit_reason.basic != EXIT_REASON_TRIPLE_FAULT &&
		     !IS_TDX_ERR_SUCCESS(vp_enter_ret));
...
#define MSG_ALLPKG	"All packages need to have online CPU to create TD. Online CPU and retry.\n"
	...
	if (IS_TDX_ERR_RND_NO_ENTROPY(err)) {
		ret = -EAGAIN;
		goto free_packages;
	}
	...
	if (IS_TDX_ERR_OPERAND_INVALID(err)) {
		/*
...
static int tdx_td_finalize(struct kvm *kvm, struct kvm_tdx_cmd *cmd)
{
	...
	if (IS_TDX_ERR_OPERAND_BUSY(cmd->hw_error))
		return -EBUSY;
				  ...
				  void __user *src, int order, void *_arg)
{
	...
	if (err) {
		ret = unlikely(IS_TDX_ERR_OPERAND_BUSY(err)) ? -EBUSY : -EIO;
		goto out;
	}
