					      ...
					      unsigned long reason)
{
	...
	if (!huge_pte_write(pte) && (reason & VM_UFFD_WP))
		return true;
	/*
	 * PTE is still RW-protected (protnone with uffd-wp marker), wait for
	 * resolution. Plain PROT_NONE without the marker is not an RWP fault.
	 */
	if (pte_protnone(pte) && huge_pte_uffd_wp(pte) && (reason & VM_UFFD_RWP))
		return true;
...
again:
	...
	if (pmd_trans_huge(_pmd)) {
		if (!pmd_write(_pmd) && (reason & VM_UFFD_WP))
			return true;
		if (pmd_protnone(_pmd) && pmd_uffd_wp(_pmd) &&
		    (reason & VM_UFFD_RWP))
			return true;
		return false;
	}
	 ...
	 * an RWP fault.
	 */
	if (pte_protnone(ptent) && pte_uffd_wp(ptent) &&
	    (reason & VM_UFFD_RWP))
		goto out;
				...
				unsigned long arg)
{
	...
	if (uffdio_register.mode & UFFDIO_REGISTER_MODE_RWP) {
		if (!pgtable_supports_uffd_wp() || VM_UFFD_RWP == VM_NONE)
			goto out;
	...
	/*
	 * WP and RWP share the uffd-wp PTE bit / marker infrastructure and
...
static __u64 uffd_api_available_features(void)
{
	 ...
	 * pgtable_supports_uffd_wp() check covers runtime (e.g. riscv
	 * without the SVRSW60T59B extension) where the PTE bit is declared
	 * but not actually usable.
	 */
	if (VM_UFFD_RWP == VM_NONE || !pgtable_supports_uffd_wp())
