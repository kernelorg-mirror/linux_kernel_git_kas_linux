					      ...
					      pmd_t *pmd, unsigned long addr)
{
	...
	for (tries = 0; tries <= COLLAPSE_FAULTIN_TRIES; tries++) {
		...
		if (pte_none(ptent) || pte_is_zero_page(ptent))
			break;
					    ...
					    unsigned int i, unsigned int bound)
{
	...
	for (nr = 1; nr < nr_max; nr++) {
		...
		if (pte_none(saved) || pte_is_zero_page(saved) ||
					...
					unsigned int nr_frozen)
{
	...
	while (i < nr_saved) {
		...
		folio = page_folio(pte_page(saved));
				       ...
				       gfp_t gfp)
{
	...
	if (unlikely(mem_cgroup_charge(folio, mm, gfp)) ||
	    folio_memcg_alloc_deferred(folio, gfp)) {
		folio_put(folio);
			  ...
			  struct collapse_control *cc)
{
	...
	for (i = 0; i < cc->nr_candidates; i++) {
		...
		for (k = 0; k < nr_pages; k++, addr += PAGE_SIZE) {
			...
			if (pte_none(cand->saved_ptes[k]) ||
			    pte_is_zero_page(cand->saved_ptes[k])) {
				     ...
				     pte_t *pte)
{
	...
	for (i = 0; i < nr_pages; i += nr, addr += nr * PAGE_SIZE) {
		...
		if (is_zero_pfn(pte_pfn(saved))) {
			/*
			 * A slot the abort cannot restore -- refilled, or its
			 * table gone -- has lost its zeropage for good.  No zap
			 * will account for what the freeze already cleared.
			 */
...
/*
 * Settle the mm's counters for a candidate the install just published.
 *
 * Slots with no source gain anon memory that no zap accounted for.
 *
 * A slot that held a KSM zeropage loses it here.  The freeze cleared that PTE
 * and the install put the destination in its place, so the decrement a zap or a
 * CoW would have done never comes.  The only other way out is an abort that
 * restores the saved value, and that leaves both the mapping and the count as
 * they were.
 */
static void collapse_account_install(struct mm_struct *mm,
				     struct collapse_candidate *cand,
				 ...
				 struct collapse_control *cc, pmd_t *pmd)
{
	...
	collapse_account_install(mm, cand, nr_populated);
	cand->deposit = NULL;
			     ...
			     struct collapse_control *cc)
{
	...
	for (i = 0; i < cc->nr_candidates; i++) {
		...
		while (k < nr_pages) {
			...
			folio = page_folio(pte_page(cand->saved_ptes[k]));
					    ...
					    struct collapse_control *cc)
{
	 ...
	 * from the whole range.  It expects the page table lock to keep the
	 * range from being unfolded, which this walk does not hold.
	 */
	...
	for (i = first_offset, addr = start; addr < end;
	     i++, addr += PAGE_SIZE) {
		pte_t pteval = ptep_get_lockless(pte + (i - first_offset));
		struct folio *folio;
		struct page *page;
		int node;
		...
		if (pte_none(pteval) || pte_is_zero_page(pteval)) {
