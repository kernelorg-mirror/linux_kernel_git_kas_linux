		...
		if (unlikely(softleaf_is_hwpoison(softleaf))) {
			...
			if (!softleaf_is_migration_read(softleaf) && cow) {
				...
				if ((userfaultfd_wp(src_vma) ||
				     userfaultfd_rwp(src_vma)) && uffd_wp)
			...
			unsigned long address, unsigned int flags)
{
	...
	/*
	 * Protnone hugetlb PTEs with the uffd-wp marker are used by
	...
	if (pte_protnone(vmf.orig_pte) && vma_is_accessible(vma) &&
	    userfaultfd_rwp(vma) && huge_pte_uffd_wp(vmf.orig_pte)) {
		spinlock_t *ptl;
		pte_t pte;
		...
		if (pte_protnone(pte) && huge_pte_uffd_wp(pte)) {
			unsigned int shift = huge_page_shift(h);

			pte = huge_pte_modify(pte, vma->vm_page_prot);
			pte = arch_make_huge_pte(pte, shift, vma->vm_flags);
			/* huge_pte_modify() preserves _PAGE_UFFD_WP; drop it on resolution */
			pte = huge_pte_clear_uffd_wp(pte);
			pte = pte_mkyoung(pte);
		...
		pgprot_t newprot, unsigned long cp_flags)
{
	...
	for (; address < end; address += psize) {
		...
		if (!ptep) {
			 ...
			 * RWP does not install markers for unpopulated
			 * entries, so skip allocation for uffd_rwp.
			 */
		...
		if (unlikely(softleaf_is_hwpoison(entry))) {
			...
			/* Already protnone with uffd-wp marker? Nothing to do. */
			if (uffd_rwp && pte_protnone(pte) && huge_pte_uffd_wp(pte))
				goto next;
			...
			if (userfaultfd_rwp(vma) && huge_pte_uffd_wp(pte))
