...
struct uffdio_api {
	 ...
	 * to the userfaultfd handler.  Use PAGEMAP_SCAN with
	 * PAGE_IS_PROT_NONE to find pages that were not re-accessed.
	 */
...
struct uffdio_rwprotect {
	struct uffdio_range range;
/*
 * UFFDIO_RWPROTECT_MODE_RWP: set the flag to read-write protect a range
 * (PROT_NONE), unset the flag to undo protection.
 *
 * UFFDIO_RWPROTECT_MODE_DONTWAKE: set the flag to avoid waking up
 * any wait thread after the operation succeeds.
 */
...
struct uffdio_set_mode {
	 ...
	 * Supported: UFFD_FEATURE_WP_ASYNC, UFFD_FEATURE_RWP_ASYNC.
