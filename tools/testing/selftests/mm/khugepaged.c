					 ...
					 const char *name)
{
	...
	if (!khugepaged_full_pass(MTHP_PASS_TIMEOUT_S))
		fail("Timeout");
	else if (all_windows_at_order(p, size))
		success("OK");
	else
		fail("Fail");
					   ...
					   struct mem_ops *ops)
{
	...
	if (!khugepaged_full_pass(MTHP_PASS_TIMEOUT_S))
		fail("Timeout");
	else if (all_windows_at_order(p + window, hpage_pmd_size - window) &&
		 !any_window_at_order(p, window) &&
		 /* Left alone at every order, not just the target one */
		 is_range_backed_by_order(p, page_size, 0,
						 pagemap_fd, kpageflags_fd))
		success("OK");
	else
		fail("Fail");
					  ...
					  struct mem_ops *ops)
{
	...
	if (!fork()) {
		...
		if (!khugepaged_full_pass(MTHP_PASS_TIMEOUT_S))
			fail("Timeout");
		else if (!any_window_at_order(p, size))
			success("OK");
		else
			fail("Fail");
