					  ...
					  uffd_test_args_t *args)
{
	...
	pm_arg.vec_len = 16;
	pm_arg.category_mask = PAGE_IS_PROT_NONE;
	pm_arg.return_mask = PAGE_IS_PROT_NONE;
 ...
 */
static void uffd_rwp_wp_coexist_test(uffd_global_test_opts_t *gopts,
				     uffd_test_args_t *args)
{
	unsigned long nr_pages = gopts->nr_pages;
	unsigned long page_size = gopts->page_size;
	...
	uargs.gopts = gopts;
	uargs.handle_fault = uffd_handle_page_fault;

	/* Populate all pages */
	for (p = 0; p < nr_pages; p++)
		memset(gopts->area_dst + p * page_size, p % 255 + 1, page_size);

	/* Register with BOTH MODE_WP and MODE_RWP */
	...
	if (ioctl(gopts->uffd, UFFDIO_REGISTER, &reg))
		err("register WP|RWP failed");

	/* Apply WP — marks pages write-protected (uffd-wp bit set) */
	wp_range(gopts->uffd, (uint64_t)gopts->area_dst,
		 nr_pages * page_size, true);

	/* Apply RWP — sets protnone while preserving the uffd-wp bit */
	rwprotect_range(gopts->uffd, (uint64_t)gopts->area_dst,
			 nr_pages * page_size, true);

	/* Read all pages — async RWP auto-resolves without faults */
	for (p = 0; p < nr_pages; p++) {
		volatile unsigned char *page =
			(unsigned char *)gopts->area_dst + p * page_size;
		unsigned char expected = p % 255 + 1;

		if (*page != expected) {
			uffd_test_fail("page %lu content mismatch after read", p);
			return;
		}
	}

	/* Start handler thread for WP faults */
	if (pthread_create(&uffd_mon, NULL, uffd_poll_thread, &uargs))
		err("uffd_poll_thread create");

	/* Write all pages — must deliver WP faults (WP bit survived RWP) */
	for (p = 0; p < nr_pages; p++) {
		unsigned char *page =
			(unsigned char *)gopts->area_dst + p * page_size;
		page[0] = 0xab;
	...
	if (uargs.wp_faults != nr_pages) {
		uffd_test_fail("expected %lu WP faults, got %llu",
			       nr_pages, uargs.wp_faults);
		goto out;
	}
...
uffd_test_case_t uffd_tests[] = {
	...
	{
		.name = "rwp-wp-coexist",
		.uffd_fn = uffd_rwp_wp_coexist_test,
		.mem_targets = MEM_ALL,
		.uffd_feature_required =
		UFFD_FEATURE_RWP | UFFD_FEATURE_RWP_ASYNC |
		UFFD_FEATURE_PAGEFAULT_FLAG_WP |
