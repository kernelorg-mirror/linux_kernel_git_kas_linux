 ...
 * attributed to it by the candidate install events at its address and
 * order, step after step.
 *
...
/* Count the candidate install events at the window's address and order */
static int count_attributed(unsigned long addr, unsigned int order)
{
	...
	while (fgets(line, sizeof(line), fp)) {
		...
		if (!s)
			continue;
		if (!strstr(s, "pass=install") || !strstr(s, "result=succeeded"))
			continue;
		o = strstr(s, "addr=");
		if (!o || sscanf(o, "addr=0x%lx", &val) != 1 || val != addr)
			continue;
		o = strstr(s, "order=");
...
static void one_step(int iteration)
{
	...
	for (i = 0; i < nr_pages; i++) {
		p[i * getpagesize()] = i + 1;
		if (pagemap_get_pfn(pagemap_fd, p + i * getpagesize()) == -1UL)
